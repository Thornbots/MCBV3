...
public:
    ...
    void update() override {
        if (autoDriveTrigger.getAsBoolean() && drivers->refSerial.getRefSerialReceivingData() && (drivers->refSerial.getGameData().gameType == RefSerialData::Rx::GameType::ROBOMASTER_RMUL_3V3)) {
            if (drivers->refSerial.getGameData().gameStage == RefSerialData::Rx::GameStage::IN_GAME) {
                ...
                //     count = 10000;
                if (!drivers->commandScheduler.isCommandScheduled(&testMoveCommand)) {
                    drivers->commandScheduler.addCommand(&testMoveCommand);
                }
            }
        }
        //  count--;
    ...
    commands::MoveToPositionCommand b0{drivers, &drivetrain, &gimbal, Pose2d(0.0f, 3.5f, 0.0f), 0.3f};
    commands::MoveToPositionCommand b1{drivers, &drivetrain, &gimbal, Pose2d(0.0f, 0.0f, 0.0f), 0.3f};
    ...
    SequentialCommand<2> testMoveCommand{{&b0, &b1}};
